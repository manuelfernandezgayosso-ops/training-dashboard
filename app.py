"""
Training Dashboard - Web Application
Flask app served on Render.com
Races: Ironman 70.3 (Jun 28 2026) + Full Marathon (Oct 4 2026)
"""
...
@app.route("/api/goals")
def api_goals():
    return jsonify(load_json("race_goals.json", {}))
...
def api_debug():
    files = {}
    if DATA_DIR.exists():
        files = {f.name: f.stat().st_size for f in DATA_DIR.iterdir()}
    return jsonify({
